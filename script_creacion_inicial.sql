...
FROM [GD1C2025].[gd_esquema].[Maestra] m
JOIN REJUNTE_SA.Factura f
ON f.id = m.Factura_Numero
JOIN REJUNTE_SA.Pedido p
ON p.id = m.Pedido_Numero AND p.fecha = m.Pedido_Fecha AND p.total = m.Pedido_Total
JOIN REJUNTE_SA.Detalle_Pedido dp
ON dp.precio = m.Detalle_Pedido_Precio AND dp.subtotal = m.Detalle_Pedido_SubTotal AND dp.cantidad = m.Detalle_Pedido_Cantidad
